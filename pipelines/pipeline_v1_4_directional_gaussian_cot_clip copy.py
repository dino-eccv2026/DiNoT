...
class StableDiffusionNDPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image = self.vae.decode(latents, return_dict=False)[0]
        # detail = [prompt]
        dir = f"sdcothd/{prompt.replace(' ', '_')}/{generator.initial_seed()}"
        import os
        os.makedirs(dir, exist_ok=True)
        image = image.to(self.vqa_model_device)
        # vqa_score = self.vqa_model(image, detail)
        # individual_vqa_score = [self.vqa_model(image, [p]) for p in decomposed_prompts]
        individual_vqa_score = []
        for p in decomposed_prompts:
            w1 = 0.8
            # individual_vqa_score.append(self.vqa_model(image, [p]))
            # print(image.shape)
            vqa_score_tmp = self.vqa_model(image, [p])
            clip_score_tmp = self.clip_model(image, [p])
            combined_score_tmp = w1 * vqa_score_tmp + (1 - w1) * clip_score_tmp
            individual_vqa_score.append(combined_score_tmp)
            print(f"decomposed prompt: {p}, vqa score: {vqa_score_tmp.item()}, clip score: {clip_score_tmp.item()}, combined score: {combined_score_tmp.item()}")

        vqa_score = sum(individual_vqa_score) / len(individual_vqa_score)
        ...
        for i in range(optimization_epoch):
            ...
            noise_pool = []
            pool_size = 10  # Number of noise candidates to sample
            ...
            with torch.no_grad():
                for noise_candidate in noise_pool:
                    ...
                    # Get the actual VQA score for this candidate
                    individual_vqa_score_tmp = [self.vqa_model(image_fwd, [p]) for p in decomposed_prompts]
                    vqa_score_tmp = sum(individual_vqa_score_tmp) / len(individual_vqa_score_tmp)

                    scores.append(vqa_score_tmp.item())

            # 3. Select the noise that maximizes the 1st-order score
            best_noise = noise_pool[np.argmax(scores)]
            ...
            vqa_score = sum(individual_vqa_score) / len(individual_vqa_score)
            print(f"vqa_score in epoch {i + 1}: {vqa_score.item()}")
            print(f"individual score: {', '.join([str(x.item()) for x in individual_vqa_score])}")
