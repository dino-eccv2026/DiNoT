...
class StableDiffusionNDPipeline(
    ...
    # def update_noise(vqa_score, clip_score, max_vqa_score, max_clip_score, alpha, beta, weight):
    #     """
    #     Modify noise based on VQA and CLIP scores separately 
    #     and combine them with a weighted sum as root(1-weight)*modified_noise1 + root(weight)*modified_noise2.
    ...
    def init_clip_model(self, clip_model, device):
        self.clip_model = clip_model
        self.clip_model_device = device
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def denoise(latents, prompt_embeds = prompt_embeds):
            # Ensure latents are real-valued floats
            if torch.is_complex(latents):
                latents = latents.real
        ...
        image = self.vae.decode(latents, return_dict=False)[0]
        detail = [prompt]
        # dir = f"sd44/{prompt.replace(' ', '_')}/{generator.initial_seed()}"
        # import os
        # os.makedirs(dir, exist_ok=True)
        image = image.to(self.vqa_model_device)
        vqa_score = self.vqa_model(image, detail)
        clip_score = self.clip_model(image, detail)
        combined_score = w1 * vqa_score + (1 - w1) * clip_score
        print(f"vqa_score: {vqa_score.item()}, clip_score: {clip_score.item()}, combined_score: {combined_score.item()}")
        max_score = combined_score.item()
        target = image
        ...
        for i in range(optimization_epoch):
            ...
            # Use combined score for backpropagation
            combined_score.backward()
            step_lr = 1 - combined_score.item() ** (1 / 2)
            # raw_step_lr = abs(1 - combined_score.item() ** 0.5)
            # clamp step_lr to [0,1] to avoid negative sqrt
            # step_lr = min(max(raw_step_lr, 0.0), 1.0)

            grad = latents_init.grad
            if grad is None:
                print(f"Warning: No gradient at epoch {i+1}, skipping")
                continue
                
            # Ensure gradients are real-valued
            ...
            beta = 1.0
            alpha = (n/(n-2))*beta  # 90% of theoretical to ensure numerical safety
            ...
            if grad_norm > 1e-6:
                ...
                noise = torch.randn_like(grad)  # fallback if grad too small
                
            latents_tmp = latents_init.detach()
            latents_tmp = (1 - step_lr) ** 0.5 * latents_tmp + step_lr ** 0.5 * noise.to(latents_tmp.dtype)
            # print(f"step_lr - {step_lr}")

            # Create new latents_init for next iteration
            latents_init = latents_tmp.detach().requires_grad_(True)
            ...
            clip_score = self.clip_model(image, detail)
            combined_score = w1 * vqa_score + (1 - w1) * clip_score
            print(f"epoch {i + 1}, vqa_score: {vqa_score.item()}, clip_score: {clip_score.item()}, combined_score: {combined_score.item()}")
            
            if combined_score.item() > max_score:
                print("update target")
                target = image
                max_score = combined_score.item()
            image = self.image_processor.postprocess(image.detach().cpu(), output_type=output_type)[0]
