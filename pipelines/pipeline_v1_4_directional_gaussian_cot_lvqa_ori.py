...
class StableDiffusionNDPipeline(
    ...
    @staticmethod
    def directional_gaussian_torch(grad, alpha, beta):
        """
    ...
    def init_ldino(
        self,
        use_grounded_sam: bool = False,  # Set False by default for compatibility
        ...
        """
        Initialize L-DINO-CoT optimizer for localized VQA scoring.
        ...
        Args:
            ...
            lambda_leak: Weight for leakage loss (distractor attributes)
            warmup_ratio: Fraction of steps using soft masks
            mask_cache_interval: How often to recompute masks
        """
        if not LVQA_AVAILABLE:
            print("Warning: lvqa_dino module not available. Using standard VQA scoring.")
            return
        ...
        print("L-DINO-CoT optimizer initialized!")
    ...
    def __call__(
        ...
        entity_attributes: Optional[Dict[str, List[str]]] = None,  # L-DINO-CoT: {"entity": ["attr1", "attr2"]}
        ...
        **kwargs,
        ...
        image = self.vae.decode(latents, return_dict=False)[0]
        # detail = [prompt]
        dir = f"sdcothdlvqa_exp1try1/{prompt.replace(' ', '_')}/{generator.initial_seed()}"
        import os
        os.makedirs(dir, exist_ok=True)
        image = image.to(self.vqa_model_device)
        
        # L-DINO-CoT: Always use localized scoring when entity_attributes are provided
        entities = None
        use_ldino = entity_attributes is not None and len(entity_attributes) > 0
        ...
        # No prompt limit needed - gradients computed per-prompt and stored on CPU
        print(f"[Gradient] Processing {len(decomposed_prompts)} decomposed prompts")
        
        # ============== PER-PROMPT GRADIENT COMPUTATION ==============
        # For each decomposed prompt:
        #   1. Compute VQA score
        #   2. Backpropagate to get gradient
        #   3. Store gradient on CPU
        #   4. Clear GPU memory
        # Then average all gradients for optimization
        
        gradients_cpu = []  # Store gradients on CPU
        individual_vqa_scores_cpu = []  # Store scalar scores
        
        for p in decomposed_prompts:
            ...
            image_for_vqa = image_for_vqa.to(self.vqa_model_device)
            
            # Compute VQA score for this prompt
            score = self.vqa_model(image_for_vqa, [p])
            score_val = score.item()
            individual_vqa_scores_cpu.append(score_val)
            print(f"decomposed prompt: {p}, score: {score_val}")
            
            # Backpropagate to get gradient for this prompt
            score.backward()
            
            # Store gradient on CPU immediately
            grad_cpu = latents_init.grad.detach().cpu().clone()
            gradients_cpu.append(grad_cpu)
            
            # Clear GPU memory
            latents_init.grad = None
            del score, image_for_vqa, latents_decoded
            gc.collect()
            torch.cuda.empty_cache()
        
        # Average all gradients on CPU
        avg_gradient = torch.stack(gradients_cpu).mean(dim=0)
        print(f"[Gradient] Averaged {len(gradients_cpu)} gradients")
        
        # Move averaged gradient back to GPU and set on latents_init
        latents_init.grad = avg_gradient.to(latents_init.device)
        del gradients_cpu, avg_gradient
        gc.collect()
        torch.cuda.empty_cache()
        
        # Compute final VQA score for logging (no gradient needed)
        avg_vqa_score = sum(individual_vqa_scores_cpu) / len(individual_vqa_scores_cpu)
        
        # Regenerate image for L-DINO and saving (no gradients needed)
        with torch.no_grad():
        ...
        # Create dummy vqa_score tensor for compatibility with rest of code
        vqa_score = torch.tensor(avg_vqa_score, device=self.vqa_model_device, requires_grad=False)
        individual_vqa_score = [torch.tensor(s, device=self.vqa_model_device) for s in individual_vqa_scores_cpu]
        # ==============================================================
        ...
        if use_ldino:
            ...
            if not hasattr(self, 'ldino_optimizer') or self.ldino_optimizer is None:
                ...
                print("[L-DINO-CoT] Optimizer auto-initialized!")
            
            entities = self.ldino_optimizer.setup_entities(
                prompt, entity_attributes, output_dir=dir
            )
            self.ldino_optimizer.print_entity_summary(entities)
            print("\n[L-DINO-CoT] Using localized VQA scoring")
            
            with torch.no_grad():
                localized_loss, entity_info = self.ldino_optimizer.compute_localized_loss(
                    image.detach(), self.image_processor, 
                    step=0, total_steps=optimization_epoch, 
                    entities=entities, save_prefix="init"
                )
            
            print(f"[L-DINO-CoT] Entity-level info:")
            for info in entity_info:
                ref_score = info.get('reflection_score', 0)
                leak_score = info.get('leakage_score', 0)
                if isinstance(ref_score, (int, float)):
                    print(f"  {info['entity']}: ref={ref_score:.3f}, leak={leak_score:.3f}")
                else:
                    print(f"  {info['entity']}: ref={ref_score}, leak={leak_score}")
            
            # Unload Grounded-SAM to free GPU memory for optimization loop
            if hasattr(self.ldino_optimizer, 'segmenter') and self.ldino_optimizer.segmenter is not None:
                if hasattr(self.ldino_optimizer.segmenter, 'model') and self.ldino_optimizer.segmenter.model is not None:
                    print("[Memory] Unloading Grounded-SAM to free GPU memory...")
                    del self.ldino_optimizer.segmenter.model
                    self.ldino_optimizer.segmenter.model = None
                    gc.collect()
                    torch.cuda.empty_cache()
        ...
        target = image.detach().clone()  # Clone to avoid reference issues
        ...
        for i in range(optimization_epoch):
            # Use stored gradient (from initial computation or previous iteration)
            ...
            beta = 1.0
            alpha = beta  # 90% of theoretical to ensure numerical safety

            noise_pool = []
            pool_size = 50  # Number of noise candidates to sample

            if grad_norm > 1e-6:
                # Preferred: fully directional, using only grad direction
                # 1. Generate a pool of 10 directional noise samples
                for _ in range(pool_size):
                    noise_sample = self.directional_gaussian_torch(grad_flat, alpha=alpha, beta=beta)
                    noise_pool.append(noise_sample.view(grad.shape))
            else:
                # Fallback: if grad is too small, use standard random noise
                for _ in range(pool_size):
                    noise_sample = torch.randn_like(grad)
                    noise_pool.append(noise_sample)

            # scores = []
            
            # 2. Score each noise candidate in the pool
            # We use the 1st-order Taylor term (gradient alignment), which is the
            # primary optimization goal.
            # for noise_candidate in noise_pool:
            #     # This is the update vector 'v' from the Taylor expansion
            #     vec = ((1 - step_lr) ** 0.5 - 1) * latents_init.detach() + step_lr ** 0.5 * noise_candidate
                
            #     # 'grad' is from vqa_score.backward() right before this block
            #     # This score is ∇s(z) * v
            #     first_order_score = (grad.detach() * vec).sum()
                
            #     scores.append(first_order_score.item())
            
            scores = []

            # 2. Score each noise candidate by running a full forward pass
            # This directly computes s(z'_T) instead of approximating it
            # We run this in no_grad() to save memory
            with torch.no_grad():
                for noise_candidate in noise_pool:
                    vec = ((1 - step_lr) ** 0.5 - 1) * latents_init.detach() + step_lr ** 0.5 * noise_candidate
                    sim = (grad.detach() * vec).sum() / vec.norm(2)**2
                    scores.append(sim.item())

            # select top-5 noise from above noise pool
            top_5_noise_idx = np.argsort(scores)[-5:]
            top_5_noise = [noise_pool[i] for i in top_5_noise_idx]
            scores = []
            
            with torch.no_grad():
                for noise_candidate in top_5_noise:
                    # Calculate the potential new latent
                    latents_tmp = (1 - step_lr) ** 0.5 * latents_init.detach() + step_lr ** 0.5 * noise_candidate

                    # Run the full generation process for this candidate
                    noise_list_tmp = denoise(latents_tmp)
                    self.scheduler.set_timesteps(num_inference_steps)

                    # We must not use .requires_grad() here
                    latents_fwd = reverse(latents_tmp, noise_list_tmp)
                    self.scheduler.set_timesteps(num_inference_steps)
                    latents_fwd = latents_fwd / self.vae.config.scaling_factor
                    image_fwd = self.vae.decode(latents_fwd, return_dict=False)[0]
                    image_fwd = image_fwd.to(self.vqa_model_device)

                    # Get the actual VQA score for this candidate (with cleanup)
                    vqa_score_tmp_sum = 0.0
                    for p in decomposed_prompts:
                        s = self.vqa_model(image_fwd, [p])
                        vqa_score_tmp_sum += s.item()
                        del s
                    vqa_score_tmp = vqa_score_tmp_sum / len(decomposed_prompts)

                    scores.append(vqa_score_tmp)
                    
                    # Memory cleanup after each candidate
                    del latents_tmp, noise_list_tmp, latents_fwd, image_fwd
                    torch.cuda.empty_cache()

            # 3. Select the noise that maximizes the 1st-order score
            best_noise = top_5_noise[np.argmax(scores)]
            ...
            for p in decomposed_prompts:
                ...
                # Compute VQA score for this prompt
                s = self.vqa_model(image, [p])
                s_val = s.item()
                individual_vqa_scores_epoch.append(s_val)
                
                # Backpropagate to get gradient
                s.backward(retain_graph=True)  # Need retain_graph for multiple backprops
                
                # Store gradient on CPU
                grad_cpu = latents_init.grad.detach().cpu().clone()
                gradients_cpu_epoch.append(grad_cpu)
            ...
            avg_gradient_epoch = torch.stack(gradients_cpu_epoch).mean(dim=0)
            latents_init.grad = avg_gradient_epoch.to(latents_init.device)
            
            # Update stored_grad for next iteration
            stored_grad = latents_init.grad.clone()
            
            del gradients_cpu_epoch, avg_gradient_epoch
            gc.collect()
            torch.cuda.empty_cache()
            
            # Compute average score for logging
            avg_score_epoch = sum(individual_vqa_scores_epoch) / len(individual_vqa_scores_epoch)
            vqa_score = torch.tensor(avg_score_epoch, device=self.vqa_model_device, requires_grad=False)
            individual_vqa_score = [torch.tensor(s, device=self.vqa_model_device) for s in individual_vqa_scores_epoch]
            
            print(f"vqa_score in epoch {i + 1}: {avg_score_epoch}")
            print(f"individual score: {', '.join([str(x) for x in individual_vqa_scores_epoch])}")
            # ==================================================================================
            ...
            if use_ldino and entities is not None:
                ...
                print(f"[L-DINO-CoT] Epoch {i+1} entity scores:")
                for info in entity_info:
                    ref = info.get('reflection_score', 0)
                    leak = info.get('leakage_score', 0)
                    print(f"  {info['entity']}: ref={ref:.3f}, leak={leak:.3f}")
