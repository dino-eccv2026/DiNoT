...
class StableDiffusionNDPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image = self.vae.decode(latents, return_dict=False)[0]
        dir = f"darwbench_imgs_f_detail/{prompt[:25].replace(' ', '_')}/{generator.initial_seed()}"
        import os, copy, gc
        save_debug = False
        ...
        for n_id, p in graph_evaluator.questions.items():
            ...
            # Apply Segmentation & Blur if L-DINO is active
            target_image = image_for_vqa
            matched_entity = None
            mask_np = None
            
            if use_ldino:
                # Heuristic: Match prompt 'p' to an entity name
                # Look up concept for better matching instead of question
                ...
                if matched_entity and matched_entity in initial_masks:
                    ...
                    if mask_np is not None:
                        # Convert mask to tensor [1, 1, H, W] to match image [1, 3, H, W]
                        mask_tensor = torch.from_numpy(mask_np).to(self.vqa_model_device).float()
                        mask_tensor = mask_tensor.unsqueeze(0).unsqueeze(0) 
                        # save image_for_vqa
                        tpath = f'{dir}/decomposed_prompt_images'
                        os.makedirs(tpath, exist_ok=True)
                        # Apply differentiable blur (preserves grad on image_for_vqa)
                        target_image = apply_blur_mask(image_for_vqa, mask_tensor, blur_radius=21)
                        target_image_pil = self.image_processor.postprocess(target_image.detach().cpu(), output_type="pil")[0]
                        target_image_pil.save(f'{tpath}/{concept}.png')
        ...
        for n_id, g in gradients_cpu:
            raw_val = raw_scores_map.get(n_id, 0.0)
            raw_val_mod = max(raw_val, 1e-9)
            mask_val = masked_scores.get(n_id, 0.0) / raw_val_mod
            coeff = mask_val
            node_grad = coeff * g
        ...
        individual_vqa_score = [torch.tensor(s, device=self.vqa_model_device) for n_id, s in individual_vqa_scores_cpu]
        # ==============================================================
        ...
        for i in range(optimization_epoch):
            ...
            for n_id, p in graph_evaluator.questions.items():
                ...
                if use_ldino:
                    ...
                    if matched_entity and matched_entity in current_masks:
                        ...
                        if mask_np is not None:
                            # Object appeared! Switch to VQA
                            mask_tensor = torch.from_numpy(mask_np).to(self.vqa_model_device).float()
                            mask_tensor = mask_tensor.unsqueeze(0).unsqueeze(0)
                            target_image = apply_blur_mask(image, mask_tensor, blur_radius=21)
            
                s = self.vqa_model(target_image, [p])
                s_val = s.item()
            ...
            for root_id, tree_score in root_scores_epoch.items():
                ...
                for n_id, g in gradients_cpu_epoch:
                    if n_id in tree_nodes:
                        coeff = 1.0 * masked_scores_epoch.get(n_id, 0.0) / max(raw_scores_map_epoch.get(n_id, 1e-9), 1e-9)
                        for on_id in tree_nodes:
                            if on_id != n_id:
                                coeff *= max(masked_scores_epoch.get(on_id, 0.0), 1e-9)
                        tree_grad += coeff * g
            ...
            individual_vqa_score = [torch.tensor(s, device=self.vqa_model_device) for n_id, s in individual_vqa_scores_epoch]
