...
class StableDiffusionNDPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image = self.vae.decode(latents, return_dict=False)[0]
        dir = f"test_cot/{prompt[:25].replace(' ', '_')}/{generator.initial_seed()}"
        import os, copy, gc
        save_debug = False
        ...
        # Compute VQA score on the image
        with torch.cuda.device(self.vqa_model_device):
            score = self.vqa_model(target_image, [prompt_str])
            score_val = score.item()
            print(f"[No-CoT Initial] question: {prompt_str}, score: {score_val}")
        ...
        if latents_init.grad is not None:
            avg_gradient = latents_init.grad.detach()
            avg_gradient = torch.nan_to_num(avg_gradient, nan=0.0, posinf=0.0, neginf=0.0)
            ...
            latents_init.grad = torch.zeros_like(latents_init.detach()).to(latents_init.device)
            print("[Gradient] Warning: latents_init.grad is None")
        
        del score, image_for_vqa, target_image
        gc.collect()
        torch.cuda.empty_cache()
        ...
        for i in range(optimization_epoch):
            ...
            with torch.cuda.device(self.vqa_model_device):
                score = self.vqa_model(target_image, [prompt_str])
                s_val = score.item()
                print(f"[No-CoT Epoch {i+1}] question: {prompt_str}, score: {s_val}")
                individual_vqa_scores_epoch.append(("q1", s_val))
            ...
            if latents_init.grad is not None:
                avg_gradient_epoch = latents_init.grad.detach()
                avg_gradient_epoch = torch.nan_to_num(avg_gradient_epoch, nan=0.0, posinf=0.0, neginf=0.0)
            else: 
                avg_gradient_epoch = torch.zeros_like(latents_init.detach().cpu())
                print("[Gradient] Warning: latents_init.grad (epoch) is None")
