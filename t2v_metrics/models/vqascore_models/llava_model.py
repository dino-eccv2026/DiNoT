...
class LLaVAModel(VQAScoreModel):
    ...
    def load_images(self, images: Union[List[str], List[torch.Tensor]]) -> torch.Tensor:
        """Load the image(s), and return a tensor (after preprocessing) put on self.device
        """
        # --- FIX: Check if the input is a tensor FIRST ---
        # If the input is a list of tensors, we must apply the same transforms
        # (resizing and normalization) so that LLaVA gets the correct 336x336 distribution.
        if isinstance(images[0], torch.Tensor):
            processed_tensors = [self.tensor_transform(img) for img in images]
            return torch.stack(processed_tensors, dim=0).to(self.device)

        # --- Original logic for handling image file paths (List[str]) ---
        loaded_images = [self.image_loader(img_path) for img_path in images]

        if self.image_aspect_ratio == 'pad':
            processed_images = [expand2square(img, tuple(int(x * 255) for x in self.image_processor.image_mean)) for img in
                     loaded_images]
        else:
            processed_images = loaded_images
        
        image_tensor = [self.image_processor.preprocess(img, return_tensors='pt')['pixel_values'][0] for img in processed_images]
        
        assert all(x.shape == image_tensor[0].shape for x in image_tensor)
        return torch.stack(image_tensor, dim=0).to(self.device)
